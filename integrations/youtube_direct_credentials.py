...
def _normalise_document(raw: Any, account: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "expires_at": raw.get("expires_at") or account.get("expires_at") or "",
        "sessionInfoHealthStatus": str(raw.get("sessionInfoHealthStatus") or "").strip(),
        "renewalLastAttemptAt": str(raw.get("renewalLastAttemptAt") or "").strip(),
        "renewalLastSuccessAt": str(raw.get("renewalLastSuccessAt") or "").strip(),
...
def save_credentials_document(storage_root: Path, account: dict[str, Any], document: dict[str, Any]) -> Path:
    ...
    payload = {
        ...
        "expires_at": normalised.get("expires_at", ""),
        "sessionInfoHealthStatus": normalised.get("sessionInfoHealthStatus", ""),
        "renewalLastAttemptAt": normalised.get("renewalLastAttemptAt", ""),
        "renewalLastSuccessAt": normalised.get("renewalLastSuccessAt", ""),
