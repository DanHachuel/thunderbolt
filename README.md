...
| AI Influencers | Menu expansível com Personagens, Geração de Conteúdo IA, Motion Control, Redes Sociais, Tutorial Meta e Tutorial Supabase; Personagens aceita múltiplas imagens e documentos `.md`/`.json`, e Geração de Conteúdo IA separa Imagens e Vídeos |
...
A área **AI Influencers > Personagens** permite guardar um personagem com nome, biografia, idioma, Instagram Business ID opcional, várias imagens de referência e documentos `.md` ou `.json`. Cada asset é validado, sanitizado, deduplicado por SHA-256 e guardado separado do perfil; imagens têm pré-visualização e documentos são mostrados como Markdown ou JSON estruturado.

O backend predefinido é **Supabase**. Em **Configurações > Configuração API > API Keys > Banco de Dados Influencers**, configure o **Supabase Project URL**, a **Supabase API key** e o bucket de Storage, aplique `seed/references/ai_influencers_schema.sql` no SQL Editor e confirme Data API/RLS. Os binários ficam no bucket e a base guarda paths e metadados. A alternativa **SQLite** cria localmente `storage/state/ai_influencers.db` e `storage/influencers/`, sem depender de um serviço externo; apenas um backend fica activo de cada vez. O botão **Testar ligação do backend** é read-only.
